...
namespace Underdog.Wpf.Dialogs
{
    ...
    {
        // private readonly ILifetimeScope _containerExtension;
        ...
        protected virtual IDialogWindow CreateDialogWindow(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
                return _serviceProvider.GetService<IDialogWindow>();
            else
                return _serviceProvider.GetKeyedService<IDialogWindow>(name);
        }
        ...
        {
            var content = _serviceProvider.GetKeyedService<object>(dialogName);
